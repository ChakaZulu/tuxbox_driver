...
unsigned int enx_crc32(void *buffer, unsigned int buffer_size)
{
    ...
    enx_reg_s(CPCSRC1)->Addr = (unsigned int)buffer;
    
    /*enx_reg_s(CPCCMD)->W = 0;
    enx_reg_s(CPCCMD)->C = 1;
    enx_reg_s(CPCCMD)->P = 0;
    enx_reg_s(CPCCMD)->N = 0;
    enx_reg_s(CPCCMD)->T = 0;
    enx_reg_s(CPCCMD)->D = 0;*/

    while (buffer_size) {
    
	if (buffer_size > ENX_MAX_TRANSCATION_SIZE) {
	
	    //enx_reg_s(CPCCMD)->Len = ENX_MAX_TRANSCATION_SIZE;
	    enx_reg_16(CPCCMD) = (1 << 14) | (ENX_MAX_TRANSCATION_SIZE - 1);
	    buffer_size -= ENX_MAX_TRANSCATION_SIZE;
	    
	} else {
	
	    //enx_reg_s(CPCCMD)->Len = buffer_size;
	    enx_reg_16(CPCCMD) = (1 << 14) | (buffer_size - 1);
	    buffer_size = 0;
	    
	}
	
	//enx_reg_s(CPCCMD)->Cmd = 0;
...
static int enx_accel_init(void)
{
    ...
    printk("enx_accel: $Id: avia_gt_accel.c,v 1.1 2002/04/01 22:29:11 Jolt Exp $\n");
    
    enx_reg_s(RSTR0)->COPY = 0;
