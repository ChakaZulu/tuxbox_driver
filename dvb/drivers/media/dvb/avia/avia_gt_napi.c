/*
 * $Id: avia_gt_napi.c,v 1.182 2003/07/24 01:14:20 homar Exp $
 *
...
struct avia_gt_dmx_queue *avia_gt_napi_queue_alloc(struct dvb_demux_feed *dvbdmxfeed, void (*cb_proc)(struct avia_gt_dmx_queue *, void *))
{
	...
	if (dvbdmxfeed->type != DMX_TYPE_TS) {
		dprintk(KERN_ERR "avia_gt_napi: strange feed type %d found\n", dvbdmxfeed->type);
		return NULL;
	}
...
void avia_gt_napi_queue_callback_section(struct avia_gt_dmx_queue *queue, void *data)
{
	...
	while (bytes_avail >= 3) {
		...
		if (section_length > 4096) {
			dprintk(KERN_ERR "avia_gt_napi: pid %04x section_length %d > 4096.\n",
...
void avia_gt_napi_queue_callback_generic(struct avia_gt_dmx_queue *queue, void *data)
{
	...
	if (queue->get_data8(queue, 1) != 0x47) {
		dprintk(KERN_ERR "avia_gt_napi: lost sync on queue %d\n", queue->index);
...
int avia_gt_napi_start_feed(struct dvb_demux_feed *dvbdmxfeed)
{
	...
	if ((dvbdmxfeed->type == DMX_TYPE_TS) || (dvbdmxfeed->type == DMX_TYPE_PES)) {
		...
		dprintk(KERN_ERR "avia_gt_napi: unknown feed type %d found\n", dvbdmxfeed->type);
		return -EINVAL;
	}
...
int __init avia_gt_napi_init(void)
{
	...
	dprintk(KERN_INFO "avia_gt_napi: $Id: avia_gt_napi.c,v 1.182 2003/07/24 01:14:20 homar Exp $\n");

	if ((section = vmalloc(4096)) == NULL) {
		dprintk("avia_gt_napi: out of memory\n");
		result = -ENOMEM;
		goto init_failed_unregister_ca_dev;
	}
	...
	if (dvb_dmx_init(&demux)) {
		dprintk(KERN_ERR "avia_gt_napi: dvb_dmx_init failed\n");
		return -EFAULT;
	}
	...
	if ((result = dvb_dmxdev_init(&dmxdev, adapter)) < 0) {
		dprintk(KERN_ERR "avia_gt_napi: dvb_dmxdev_init failed (errno=%d)\n", result);
	...
	if ((result = demux.dmx.add_frontend(&demux.dmx, &fe_hw)) < 0) {
		dprintk(KERN_ERR "avia_gt_napi: add_frontend (hw) failed (errno=%d)\n", result);
	...
	if (ucode_caps & AVIA_GT_UCODE_CAP_ECD) {
		if ((result = dvb_register_device(adapter, &ca_dev, &avia_gt_napi_ecd_dev, NULL, DVB_DEVICE_CA)) < 0) {
			dprintk(KERN_ERR "avia_gt_napi: dvb_register_device failed (errno=%d)\n", result);
	...
	if (hw_dmx_sec)
		dprintk(KERN_INFO "avia_gt_napi: hw section filtering enabled.\n");
