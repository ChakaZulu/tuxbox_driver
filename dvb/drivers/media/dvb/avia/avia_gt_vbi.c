/*
 * $Id: avia_gt_vbi.c,v 1.24 2003/07/24 01:14:20 homar Exp $
 *
...
void avia_gt_vbi_irq(u16 irq)
{
	...
	if (tt_error) {
		dprintk(KERN_INFO "avia_gt_vbi: error in TS stream\n");
		avia_gt_vbi_stop();
		avia_gt_vbi_start();
	}

	if (tt_pts)
		dprintk(KERN_INFO "avia_gt_vbi: got pts\n");
}
#endif

int __init avia_gt_vbi_init(void)
{
	dprintk(KERN_INFO "avia_gt_vbi: $Id: avia_gt_vbi.c,v 1.24 2003/07/24 01:14:20 homar Exp $\n");
	...
	if (avia_gt_alloc_irq(gt_info->irq_tt, avia_gt_vbi_irq)) {
		dprintk("avia_gt_vbi: unable to get vbi interrupt\n");
		return -EIO;
	}
#endif
